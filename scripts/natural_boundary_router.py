...
OUT_JSON = OUTPUTS / "natural_boundary_router_latest.json"
OUT_JSONL = OUTPUTS / "natural_boundary_router.jsonl"
...
SOURCES = {
    ...
    "pressure_state": OUTPUTS / "pressure_regulator_latest.json",
}
...
def build_payload() -> Dict[str, Any]:
    state = build_natural_boundary_router_state(
        ...
        pressure_state=read_json(SOURCES["pressure_state"]),
    )
...
def main() -> int:
    ...
    print(json.dumps({"route": payload["routing"]["route"], "next_contact": payload["next_contact"]}, ensure_ascii=False, indent=2))
    print(OUT_JSON)
    return 0
