...
OUT_JSON = OUTPUTS / "trigger_geometry_latest.json"
OUT_JSONL = OUTPUTS / "trigger_geometry.jsonl"

SOURCES = {
    "field_trigger": OUTPUTS / "field_trigger_state_latest.json",
...
def build_payload() -> Dict[str, Any]:
    state = build_trigger_geometry_state(
        field_trigger_state=read_json(SOURCES["field_trigger"]),
        field_heart_state=read_json(SOURCES["field_heart"]),
        natural_boundary_state=read_json(SOURCES["natural_boundary"]),
        field_reseeding_state=read_json(SOURCES["field_reseeding"]),
    )
...
def main() -> int:
    ...
    print(json.dumps({"shape": payload["geometry"]["shape"], "next_contact": payload["next_contact"]}, ensure_ascii=False, indent=2))
    print(OUT_JSON)
    return 0
